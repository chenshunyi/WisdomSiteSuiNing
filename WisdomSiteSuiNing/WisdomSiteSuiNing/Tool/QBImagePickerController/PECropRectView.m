...
@property (weak, nonatomic) PEResizeControl *topLeftCornerView;
@property (weak, nonatomic) PEResizeControl *topRightCornerView;
@property (weak, nonatomic) PEResizeControl *bottomLeftCornerView;
@property (weak, nonatomic) PEResizeControl *bottomRightCornerView;
@property (weak, nonatomic) PEResizeControl *topEdgeView;
@property (weak, nonatomic) PEResizeControl *leftEdgeView;
@property (weak, nonatomic) PEResizeControl *bottomEdgeView;
@property (weak, nonatomic) PEResizeControl *rightEdgeView;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat width = CGRectGetWidth(self.bounds)-2*Border;
    CGFloat height = CGRectGetHeight(self.bounds)-2*Border;
    ...
    for (NSInteger i = 0; i < 3; i++) {
        ...
        if (self.showsGridMajor) {
            if (i > 0) {
                ...
                UIRectFill(CGRectMake(Border+roundf(width / 3 * i), Border, 1.0f, roundf(height)));
                UIRectFill(CGRectMake(Border, Border+roundf(height / 3 * i), roundf(width), 1.0f));
            }
        }
    }
    [[UIColor whiteColor] set];
    UIRectFill(CGRectMake(Border+roundf(width), Border, 1.0f, roundf(height)));
    UIRectFill(CGRectMake(Border, Border+roundf(height), roundf(width), 1.0f));
    UIRectFill(CGRectMake(Border, Border, 1.0f, roundf(height)));
    UIRectFill(CGRectMake(Border, Border, roundf(width), 1.0f));
    
    //左上//shang
    UIRectFill(CGRectMake(Border-hig, Border-hig, wid, hig));
    //左下
    UIRectFill(CGRectMake(Border-hig, Border-hig, hig, wid));
    //右上
    UIRectFill(CGRectMake(Border+width-(wid-hig), Border-hig, wid, hig));
    //右下
    UIRectFill(CGRectMake(Border+width, Border-hig, hig, wid));
    
    
    //左上//xia
    UIRectFill(CGRectMake(Border-hig, Border+height, wid, hig));
    //左下
    UIRectFill(CGRectMake(Border-hig, Border+height-(wid-hig), hig, wid));
    //右上
    UIRectFill(CGRectMake(Border+width-(wid-hig), Border+height, wid, hig));
    //右下
    UIRectFill(CGRectMake(Border+width,Border+height-(wid-hig), hig, wid));
    
    //top
    UIRectFill(CGRectMake(Border+width/2-wid/2,Border-hig, wid, hig));
    //buttom
    UIRectFill(CGRectMake(Border+width/2-wid/2,Border+height, wid, hig));
    //left
    UIRectFill(CGRectMake(Border-hig,Border+height/2-wid/2, hig, wid));
    //right
    UIRectFill(CGRectMake(Border+width,Border+height/2-wid/2, hig, wid));}
